...
| 🎭 全局人设 | 内置「全局人设」设置页：给所有工作区/会话统一一份人设词（默认关闭，可随时开启） |
| 🎵 音乐播放器 | 外接网易云单曲 / B站视频 / 音频直链，本地歌单 + 音量记忆 |
| 🎤 侧边栏歌词 | KTV 式多行歌词卡：渐变流动高亮、弹性动画、自动预位 |
| 📐 可拖动面板 | 文件树/编辑器/对话面板之间分隔条自由调宽，宽度记忆 |
| 🎬 精致启动动画 | 浅色渐变 + 扫光 + 进度条，尊重系统减弱动效 |
| 📦 内置插件 | 预置大肥鱼桌面伴侣、玻璃拟态界面、全局人设等插件，启动自动部署注册 |
| 📊 系统信息页 | 设置 → 系统信息：显示 DSH 核心版本/客户端版本/Node/端口，一键「检查更新」「立即更新」（npm 更新通道） |
| 🔄 一键智能重启 | 「重启DSH服务.bat」重做：自动读取真实端口、防误杀校验、停服后自动拉起客户端、就绪确认并显示新版本 |
| 🚫 无浏览器双开 | 桌面窗口启动服务时自动 `--no-open`，不再额外弹出浏览器；launcher 浏览器模式不受影响 |
| ⬆️ 核心版本跟随 | 内置 DeepSeek Harness 核心引擎包跟随官方 npm 最新版（当前 0.1.1-rc.2） |
...
1. 下载 `deepseek-harness-client-v0.3.2.exe`（约 182MB，自包含）
2. 双击运行：
...
1. 下载 `deepseek-harness-client-v0.3.2.zip`（约 182MB）
